...
long_sleep = partial(sleep, 120)
generic_tick_keys = '100,101,104,106,165,221,225,233,236'
...
def save_order_id(msg):
    print "save order Server Response: %s, %s" % (msg.typeName, msg)
    order_ids.append(msg.orderId)


def save_tick(msg):
    print "save tick Server Response: %s, %s" % (msg.typeName, msg)
...
def test_002(connection, options):
    print "test getMktData"
    ticker_id = gen_tick_id()
    contract = make_contract('GOOG')
    data = connection.reqMktData(tickerId = ticker_id,
                                 contract = contract, 
                                 genericTickList = generic_tick_keys,
                                 snapshot = False)
    print data
    short_sleep()
...
def test_007(connection, options):
    print "test get history data"
    endtime = strftime('%Y%m%d %H:%M:%S')
    print endtime
    ticker_id = gen_tick_id()
    print ticker_id
    data = connection.reqHistoricalData(
        tickerId=ticker_id,
        contract=make_contract('GOOG'),
        endDateTime=endtime,
        durationStr='2 D',
...
def test_009(connection, options):
    ...
    ticker_id = gen_tick_id()
    connection.reqRealTimeBars(
        ticker_id, make_contract('AAPL'), 5, 'TRADES', 0)
    short_sleep()
